...
impl Ciphersuite for Ed25519Sha512 {
    ...
    fn H2(m: &[u8]) -> <<Self::Group as Group>::Field as Field>::Scalar {
        let output = hash_to_array(&[m]);
        let lower = &output[0..32];
        Scalar::from_bytes_mod_order(lower.try_into().unwrap())
    }

    /// H3 for FROST(Ed25519, SHA-512)
    ///
    /// [spec]: https://datatracker.ietf.org/doc/html/rfc9591#section-6.1-2.4.2.6
    fn H3(_m: &[u8]) -> <<Self::Group as Group>::Field as Field>::Scalar {
        // Set to zero to disable binding for compatibility with standard Ed25519
